...
{
	static int inited = 0;
	int kx, initStatus, tmp;

	if(inited) return 0;			/* already inited */
	...
	gblFFPoly = poly;					/* prime poly, 0: use default poly, non-zero: poly to use  */
	gblTParm =  t;						/* max correctable error bits */
	...
	if (gblFFPoly==0){ // use default poly
...
int bch_encode(uint8_t *data, unsigned int data_len, uint8_t *ecc)
{
	if(data == NULL || ecc == NULL)	return -1;
	int e, tmp;

	e = bch_init(5, 3, 55);
	tmp = (gblNParm-gblCgpDegree)/8;
	if(data_len > tmp )	return -2;

	/* update "DataBytes" and "CodewordBytes", originally it was updated when
	   asked from user's input */
	gblNumDataBytes = data_len;
	gblNumCodewordBytes = gblNumDataBytes+gblNumRedunBytes;
	gblNumDataBits = gblNumDataBytes*8;
	gblKParm = gblNumDataBits;


	/* update codeword with user data, originally it was updated in
		 genWriteData() */
	// Clear gblCodeword including pad bits
	for (int kx=0;kx<gblNumCodewordBytes;kx++){
		gblCodeword[kx] = 0; // Clear gblCodeword
	}
	// Put user data in gblCodeword - there are no pad bits in user data
	for (int kx=0;kx<gblNumDataBytes;kx++){
		gblCodeword[kx] = (data[kx] % BYTESTATES);// Reduce user data to byte size
	}
	...
	/* extract the ecc */
	for(int i = 0; i < gblNumRedunBytes; i++ )
	{
		ecc[i] = (gblCodeword[gblNumDataBytes + i] % BYTESTATES);
	}

	return e;
}

static int bch_decode(uint8_t *data_in, int data_len, uint8_t *ecc_in, uint8_t *data_out)
{
	int dcdStatus = 0, errFlag = 0;
	int e, tmp;
	...
	// Clear gblCodeword including pad bits
	for (int i = 0; i < gblNumCodewordBytes; i++)
	{
		gblCodeword[i] = 0;
	}

	// Put user data in gblCodeword
	for (int i = 0; i < gblNumDataBytes; i++)
	{
		gblCodeword[i] = (data_in[i] % BYTESTATES);
	}

	// Put ecc data in gblCodeword
	for (int i = 0; i < gblNumRedunBytes; i++)
	{
		gblCodeword[gblNumDataBytes + i] = (ecc_in[i] % BYTESTATES);
	}
	...
	dcdStatus = bchDecode(gblLoc, gblAlogTbl, gblLogTbl, gblFFSize, gblTParm,
												...
												gblNumDataBits, gblNumRedunBits, gblCodeword,
												gblNumRedunWords, gblNumRedunBytes, gblNumDataBytes, gblEncodeTbl,
												gblRemainBytes, gblSyndromes, &errFlag, gblLogZVal, gblMParm, gblFFSize);
	...
	if(dcdStatus < UNCORR)
	{
		...
		for (int i = 0; i < gblNumDataBytes; i++)
		{
			data_out[i] = (gblCodeword[i] % BYTESTATES);
		}
		e = 0;
	}
	else e = -1;
...
int main()
{
	...
	uint8_t data[2] = {0x11, 0x11};
	uint8_t ecc[2] = {0};
	uint8_t data_out[2] = {0};

	e = bch_encode(data, sizeof(data), ecc);

	printf("data: 0x%02X 0x%02X, ecc: 0x%02X 0x%02X, (%d)\n",
				 data[0], data[1], ecc[0], ecc[1], e);

	/* create 3 bit error in data and try decode */
	data[0] = 0x12;
	data[1] = 0x10;
	e = bch_decode(data, sizeof(data), ecc, data_out);
	printf("data: 0x%02X 0x%02X, ecc: 0x%02X 0x%02X, data_out: 0x%02X 0x%02X, (%d)\n",
				 data[0], data[1], ecc[0], ecc[1], data_out[0], data_out[1], e);
